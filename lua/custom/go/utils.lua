...
if nvim_exec == nil then nvim_exec = vim.api.nvim_exec end
...
function utils.is_windows() return is_windows end

function utils.ext()
  if is_windows then return '.exe' end
  return ''
end
...
local function strip_path_sep(path)
  ...
  if l == utils.sep() then return path:sub(1, #path - 1) end
  return path
end
  ...
  if p then
    ...
    for _, content in pairs(contents) do
      ...
      if content:find 'interface' then
        ...
        if iface_name ~= '' then table.insert(ifaces, pkg .. iface_name) end
      end
    end
  end
  utils.log(ifaces)
...
function utils.get_fname_num(line)
  ...
  if f[1] and f[1] ~= '' then return f[2], tonumber(f[3]) end
end
...
utils.log = function(...)
  if not _GO_NVIM_CFG or not _GO_NVIM_CFG.verbose then return end
  local l = select('#', ...)
  local arg = { ... }
  ...
  if has_packer or has_lazy then
    ...
    if has_packer then
      ...
      if not pkg[name] or not pkg[name].loaded then
        ...
        if pkg[name] ~= nil then loader(name) end
      end
    else
    ...
    if paths:find(name) then vim.cmd('packadd ' .. name) end
  end
...
function utils.chdir(dir)
  if fn.exists '*chdir' then return fn.chdir(dir) end
...
function utils.all_pkgs() return '.' .. utils.sep() .. '...' end

-- log and messages
function utils.warn(msg)
  vim.schedule(function() vim.notify('WARN: ' .. msg, vim.log.levels.WARN) end)
end

function utils.error(msg)
  vim.schedule(function() vim.notify('ERR: ' .. msg, vim.log.levels.ERROR) end)
end

function utils.info(msg)
  vim.schedule(function() vim.notify('INFO: ' .. msg, vim.log.levels.INFO) end)
end

function utils.debug(msg)
  vim.schedule(function() vim.notify('DEBUG: ' .. msg, vim.log.levels.DEBUG) end)
end
...
function utils.work_path()
  ...
  if #workfolders == 1 then return workfolders[1] end

  for _, value in pairs(workfolders) do
    local mod = value .. utils.sep() .. 'go.mod'
    if utils.file_exists(mod) then return value end
  end
...
function utils.restart(cmd_args)
  ...
  if old_lsp_client then vim.lsp.stop_client(old_lsp_client.id) end

  local configs = vim.lsp.config or require 'lspconfig.configs'
  if configs['gopls'] and configs['gopls'].launch then
    vim.defer_fn(function() configs['gopls'].launch() end, 500)
    return
  else
...
function utils.list_directory() return fn.map(fn.glob(fn.fnameescape './' .. '/{,.}*/', 1, 1), 'fnamemodify(v:val, ":h:t")') end
...
function utils.set_nulls()
  if _GO_NVIM_CFG.null_ls_document_formatting_disable then
    local query = {}
    if type(_GO_NVIM_CFG.null_ls_document_formatting_disable) ~= 'boolean' then query = _GO_NVIM_CFG.null_ls_document_formatting_disable end
    local ok, nulls = pcall(require, 'null-ls')
    if ok then nulls.disable(query) end
  end
end
...
function utils.offset(line, col)
  utils.log(line, col)
  if vim.o.encoding ~= 'utf-8' then print('only utf-8 encoding is supported current encoding: ', vim.o.encoding) end
...
function utils.get_build_tags(buf)
  ...
  for i = 1, cnt do
    ...
    if string.find(line, 'package') then break end
...
function utils.lorem() return lorem end
  ...
  if _GO_NVIM_CFG.trouble == true then
    ...
    if cmd:find 'copen' then
      ...
      for i = 1, vim.fn.winnr '$' do
        if vim.fn.getwinvar(i, '&buftype') == 'quickfix' then return end
      end
    end
    vim.cmd(cmd)
  end
...
utils.extract_filepath = function(msg, pkg_path)
  ...
  if vim.fn.executable 'find' == 0 then return false, fname, lnum end
  -- note: slow operations
  local cmd = 'find ./ -type f -name ' .. "'" .. filename .. "'"
  local path = vim.fn.systemlist(cmd)

  if vim.v.shell_error ~= 0 then utils.warn('find failed ' .. cmd .. vim.inspect(path)) end
